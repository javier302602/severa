...
describe('ImportarDatasetConAuditoria', () => {
  test('RF-99: notifica vulnerabilidad crítica cuando el CVSS importado es >= 9.0', async () => {
    ...
    const decorator = new ImportarDatasetConAuditoria(
      usecaseFalso({ importados: 1, rechazados: 0, errores: [] }),
      auditoriaRepository,
    ...
    expect(servicioDeNotificaciones.notificarVulnerabilidadCritica).toHaveBeenCalledWith(critica, 'analista-7');
  });

  test('NO notifica cuando el CVSS importado es menor a 9.0', async () => {
    ...
    expect(servicioDeNotificaciones.notificarVulnerabilidadCritica).not.toHaveBeenCalled();
  });

  test('notifica una vez por cada vulnerabilidad crítica cuando hay varias en la misma importación', async () => {
    ...
    const servicioDeNotificaciones = servicioDeNotificacionesFalso();
    const decorator = new ImportarDatasetConAuditoria(
      usecaseFalso({ importados: 3, rechazados: 0, errores: [] }),
      auditoriaFalsa(),
      servicioDeNotificaciones
    );
    ...
    expect(servicioDeNotificaciones.notificarVulnerabilidadCritica).toHaveBeenCalledTimes(2);
  });
  ...
  test('sigue registrando auditoría aunque no haya ninguna vulnerabilidad crítica', async () => {
    ...
    const servicioDeNotificaciones = servicioDeNotificacionesFalso();
    const decorator = new ImportarDatasetConAuditoria(
      usecaseFalso({ importados: 0, rechazados: 0, errores: [] }),
      auditoriaRepository,
      servicioDeNotificaciones
    );
  ...
  test('embebe el nombre de archivo en el detalle de auditoría cuando se lo pasan', async () => {
    const auditoriaRepository = auditoriaFalsa();
    const decorator = new ImportarDatasetConAuditoria(
      usecaseFalso({ importados: 2, rechazados: 1, errores: [] }),
      auditoriaRepository,
